...
describe.skip('Test ConvexCrvAlEth strategy on WETH vault', async () => {
    let vault, strategy, weth, governance, booster, wrapped, rewards;
    let snapshot;

    describe('Initial setup of contracts', async () => {
        it('Setup contracts', async () => {
            const weth_vault = await ethers.getNamedSigner('weth_vault');
            vault = await ethers.getContractAt('Vault', weth_vault.address);
            ...
            expect(weth.address.toLowerCase()).to.be.eq(vault_token.toLowerCase(), 'Bad vault token');
        });

        it('Deploy strategy', async () => {
            const Strategy = await ethers.getContractFactory('ConvexCrvAlEthStrategy');
            ...
            const wrapped_addr = await strategy.CrvAlEth();
    ...
    describe('Test strategy main functionality', async () => {
        it('1st harvest', async () => {
            const vault_bal_before = await weth.balanceOf(vault.address);
            await strategy.connect(governance).harvest();
            const vault_bal_after = await weth.balanceOf(vault.address);
        ...
        it('Vault withdraw from strategy', async () => {
            const vault_bal = await weth.balanceOf(vault.address);
            ...
            // ask 1 more weth than vault has
            const withdraw_asked = vault_bal.add('1000000000000000000');
            ...
            const alice_bal_before = await weth.balanceOf(alice.address);
            // execute with possible loss
            await vault.connect(alice)['withdraw(uint256,uint256)'](0, 0);
            const alice_bal_after = await weth.balanceOf(alice.address);
            const vault_bal_after = await weth.balanceOf(vault.address);
        ...
        it('Emergency and revoke', async() => {
            ...
            // apply 0.03% slippage because of withdrawing from h-weth pool
