...
interface VaultWrapper:
    def initialize(vault: address): nonpayable
...
event UpdateVaultWrapper:
    vaultWrapper: indexed(address)
...
def __init__(
    ...
    self.bridge = bridge
    self.vaultWrapper = vaultWrapper


@external
def setBridge(bridge: address):
    assert msg.sender == self.governance  # dev: unauthorized
    self.bridge = bridge

@external
def setVaultWrapper(vaultWrapper: address):
    assert msg.sender == self.governance  # dev: unauthorized
    self.vaultWrapper = vaultWrapper
...
def _newProxyVault(
    ...
    _vaultWrapper: address = create_forwarder_to(self.vaultWrapper)
    VaultWrapper(_vaultWrapper).initialize(vault)
