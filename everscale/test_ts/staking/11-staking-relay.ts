...
const {
    ...
    logger, mintTokens, stringToBytesArray, getRandomNonce, sleep
} = require('../utils');
    ...
    describe('Setup contracts', async function() {
        ...
        describe('Staking', async function() {
            ...
            it('Deploy staking', async function () {
                ...
                stakingWallet = await locklift.factory.getDeployedContract('TokenWallet', staking_details.tokenWallet);

                // call in order to check if wallet is deployed
                const owner_address = await stakingWallet.methods.owner({answerId: 0}).call();
                const root_address = await stakingWallet.methods.root({answerId: 0}).call();
            ...
            it('Installing codes', async function() {
                ...
                await stakingRoot.methods.installPlatformOnce({
                ...
                const active = await stakingRoot.methods.isActive({answerId: 0}).call();
            ...
            it('Sending reward tokens to staking', async function() {
                ...
                const staking_balance = await stakingWallet.methods.balance({answerId: 0}).call();
            ...
            it("Setting relay config for testing", async function() {
                ...
                const relay_config = await stakingRoot.methods.getRelayConfig({answerId: 0}).call().then(t => t.value0);
    ...
    describe('Relay pipeline testing', async function () {
        describe('Standard case', async function() {
            ...
            it("Creating origin relay round", async function () {
                const user1_pk = new BigNumber((await locklift.keystore.getSigner("0"))!.publicKey, 16);
                ...
                const total_tokens_staked = await round.methods.total_tokens_staked().call();
                const round_reward = await round.methods.round_reward().call();
                const relays_count = await round.methods.relays_count().call();
                const reward_round_num = await round.methods.reward_round_num().call();
                ...
                const relay_rounds_data = await stakingRoot.methods.getRelayRoundsDetails({answerId: 0}).call().then(t => t.value0);
                ...
                const expected_reward = round_reward + new BigNumber(reward_rounds[0].totalReward);
                expect(expected_reward.toString()).to.be.equal(reward_rounds_new[0].totalReward.toString(), "Bad reward after relay round init");

                const events = await stakingRoot.getPastEvents({filter: 'RelayRoundInitialized'}).then((e) => e.events);
                const [{
                    data: {
                ...
                const relay = await round.methods.getRelayByStakerAddress({
                _relay_staker_addr: user1.address, answerId: 0
                }).call();

                expect(relay._staker_addr).to.be.equal(user1.address, "Relay creation fail - staker addr");
                expect(relay._ton_key.toString()).to.be.equal(user1_pk.toString(16), "Relay creation fail - ton pubkey");
                expect(relay._eth_addr.toString()).to.be.equal(user1_eth.toString(16), "Relay creation fail - eth addr");
            ...
            it("Users link relay accounts", async function () {
                ...
                await linkRelayAccounts(user2, (await locklift.keystore.getSigner("1"))!.publicKey, user2_eth_addr);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }
                ...
                const user2_pk_expected = new BigNumber((await locklift.keystore.getSigner("1"))!.publicKey, 16);
                const user2_eth_addr_expected = new BigNumber(user2_eth_addr.toLowerCase(), 16);

                expect(user2_pk_expected.toString(16)).to.be.equal(user2_pk.toString(), "Bad ton pubkey installed");
                expect(_user2_eth_addr.toString()).to.be.equal(user2_eth_addr_expected.toString(16), "Bad eth addr installed");

                await linkRelayAccounts(user3, (await locklift.keystore.getSigner("2"))!.publicKey, user3_eth_addr);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }
                ...
                const user3_pk_expected = new BigNumber((await locklift.keystore.getSigner("2"))!.publicKey, 16);
                const user3_eth_addr_expected = new BigNumber(user3_eth_addr.toLowerCase(), 16);

                expect(user3_pk_expected.toString(16)).to.be.equal(user3_pk.toString(), "Bad ton pubkey installed");
                expect(_user3_eth_addr.toString()).to.be.equal(user3_eth_addr_expected.toString(16), "Bad eth addr installed");
            });
            ...
            it("Users confirm ton relay accounts", async function () {
                ...
                await confirmTonRelayAccount((await locklift.keystore.getSigner("1"))!, user2Data);
                await confirmTonRelayAccount((await locklift.keystore.getSigner("2"))!, user3Data);
            ...
            it("Election on new round starts", async function () {
                ...
                const tx = await startElection((await locklift.keystore.getSigner("1"))!);
                ...
                const events = await stakingRoot.getPastEvents({filter: 'ElectionStarted'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
            ...
            it("Users request relay membership", async function () {
                ...
                const {transaction: tx} = await requestRelayMembership((await locklift.keystore.getSigner("0"))!, user1Data);
                const election = await getElection(1);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }
                ...
                const events = await user1Data.getPastEvents({filter: 'RelayMembershipRequested'}).then((e) => e.events);
                const [{
                    data: {
                ...
                await requestRelayMembership((await locklift.keystore.getSigner("2"))!, user3Data);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }

                const events3 = await user3Data.getPastEvents({filter: 'RelayMembershipRequested'}).then((e) => e.events);
                const [{
                    data: {
                ...
                const user3_pk = new BigNumber((await locklift.keystore.getSigner("2"))!.publicKey, 16);
                ...
                await requestRelayMembership((await locklift.keystore.getSigner("1"))!, user2Data);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }

                const events2 = await user3Data.getPastEvents({filter: 'RelayMembershipRequested'}).then((e) => e.events);
                const [{
                    data: {
                ...
                const user2_pk = new BigNumber((await locklift.keystore.getSigner("1"))!.publicKey, 16);
                ...
                } = await election.methods.getRequests({limit: 10, answerId: 0}).call();
                ...
                expect(`0x${ton_keys[0].toString()}`).to.be.equal(expected_ton_pubkey3, "Bad request - ton pubkey");
                ...
                expect(`0x${ton_keys[1].toString()}`).to.be.equal(expected_ton_pubkey2, "Bad request - ton pubkey");
                ...
                expect(`0x${ton_keys[2].toString()}`).to.be.equal(expected_ton_pubkey1, "Bad request - ton pubkey");
            ...
            it("Election ends, new round initialized", async function () {
                ...
                const events = await stakingRoot.getPastEvents({filter: 'ElectionEnded'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                ...
                const events1 = await stakingRoot.getPastEvents({filter: 'RelayRoundInitialized'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num1,
                        round_start_time: _round_start_time,
                ...
                const stored_total_tokens_staked = await round.methods.total_tokens_staked().call();
                const stored_reward_round_num = await round.methods.reward_round_num().call();
                ...
                expect(stored_total_tokens_staked.toString()).to.be.equal(expected_staked_tokens.toString(16), "Bad round created - total tokens staked");
            ...
            it("User1 get reward for origin round", async function() {
                ...
                await getRewardForRelayRound((await locklift.keystore.getSigner("0"))!, user1Data, 0);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }
                ...
                const events1 = await user1Data.getPastEvents({filter: 'RelayRoundRewardClaimed'}).then((e) => e.events);
                const [{
                    data: {
        ...
        describe("Not enough relay requests on election", async function() {
            ...
            it("Election on new round starts", async function () {
                ...
                const tx = await startElection((await locklift.keystore.getSigner("2"))!);
                const election = await getElection(2);
                if (locklift.context.network.name === 'dev') {
                    await wait(DEV_WAIT);
                }
                ...
                const events1 = await stakingRoot.getPastEvents({filter: 'ElectionStarted'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                        election_start_time: _election_start_time,
            ...
            it("Users request relay membership", async function() {
                ...
                const events1 = await user1Data.getPastEvents({filter: 'RelayMembershipRequested'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num1,
                        tokens: _tokens1,
            ...
            it("Election ends, not enough users participated, clone prev. round", async function() {
                ...
                const events1 = await stakingRoot.getPastEvents({filter: 'ElectionEnded'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                        relay_requests: _relay_requests,
                ...
                const events2 = await stakingRoot.getPastEvents({filter: 'RelayRoundInitialized'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num1,
                        round_start_time: _round_start_time,
                ...
                const reward_rounds_new = staking_details_1.rewardRounds;
                // console.log(reward_rounds, reward_rounds_new, round_reward.toString());
                const expected_reward = round_reward + (new BigNumber(reward_rounds[1].totalReward));
            ...
            it("Users get reward for prev relay round", async function() {
                ...
                for (const i of [
                    ...
                    const relay = await relay_round.methods.getRelayByStakerAddress({_relay_staker_addr: _user.address, answerId: 0}).call();
                    const staked_tokens = new BigNumber(relay._staked_tokens.toString());
                    const total_tokens_staked = await relay_round.methods.total_tokens_staked().call();
                    ...
                    await getRewardForRelayRound((await locklift.keystore.getSigner("i"))!, _userData, 1);
                    if (locklift.context.network.name === 'dev') {
                        await wait(DEV_WAIT);
                    }
                    ...
                    const events2 = await _userData.getPastEvents({filter: 'RelayRoundRewardClaimed'}).then((e) => e.events);
                    const [{
                        data: {
                            relay_round_num: _relay_round_num,
                            reward_round_num: _reward_round_num,
            ...
            it("Slash user", async function() {
                ...
                const round1_rewards_data = new BigNumber(rounds_rewards_data[0]);

                const user1_reward_before = await userRewardRounds(user1Data);
                const user1_reward = new BigNumber(user1_reward_before[0].reward_balance);
                ...
                const user1_token_balance0 = (new BigNumber(await userTokenBalance(user1Data)));
                ...
                const events2 = await stakingRoot.getPastEvents({filter: 'RelaySlashed'}).then((e) => e.events);
                const [{
                    data: {
                        user: _user,
                ...
                const expected_withdraw = user1_token_balance0 + (new BigNumber(user1_token_reward));
                ...
                const prev_token_balance = (new BigNumber(staking_details_before.tokenBalance));
                const after_token_balance = (new BigNumber(staking_details_after.tokenBalance));
                expect(prev_token_balance - user1_token_balance0).to.be.equal(after_token_balance, "Bad token balance");
            });
        });
        ...
        describe("Destroy old relay round", async function() {
            it("Election on new round starts", async function () {
                ...
                const events2 = await stakingRoot.getPastEvents({filter: 'ElectionStarted'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                        election_start_time: _election_start_time,
            ...
            it("Election ends, not enough users participated, clone prev. round", async function() {
                ...
                const events2 = await stakingRoot.getPastEvents({filter: 'ElectionEnded'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                        relay_requests: _relay_requests,
                ...
                const events3 = await stakingRoot.getPastEvents({filter: 'RelayRoundInitialized'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num1,
                        round_start_time: _round_start_time,
        ...
        describe("Late round start", async function() {
            it("Election on new round starts", async function () {
                ...
                const events3 = await stakingRoot.getPastEvents({filter: 'ElectionStarted'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                        election_start_time: _election_start_time,
            ...
            it("Election ends late, not enough users participated, clone prev. round", async function() {
                ...
                const root_round_details = await stakingRoot.methods.getRelayRoundsDetails({answerId: 0}).call().then(t => t.value0);
                ...
                const events3 = await stakingRoot.getPastEvents({filter: 'ElectionEnded'}).then((e) => e.events);
                const [{
                    data: {
                        round_num: _round_num,
                        relay_requests: _relay_requests,
                ...
                const root_round_details_1 = await stakingRoot.methods.getRelayRoundsDetails({answerId: 0}).call().then(t => t.value0);
                expect(
                    root_round_details_1.currentRelayRoundStartTime
                ).to.be.gt(root_round_details.currentRelayRoundEndTime, "Bad new round start time");
            });
        });
        ...
        describe("Emergency situation", async function() {
           it("Set emergency", async function() {
            ...
            it("User withdraw tokens", async function() {
                ...
                const expected_bal = new BigNumber(prev_details.tokenBalance) - userDeposit;
                expect(after_details.tokenBalance).to.be.equal(expected_bal);
            });
           ...
           it("Rescuer withdraw all tokens", async function() {
               ...
               await sleep(1000);
               const after_details = await stakingRoot.methods.getDetails({answerId: 0}).call().then(v => v.value0);
               expect(after_details.tokenBalance).to.be.equal(new BigNumber(prev_details.tokenBalance) - 100);
