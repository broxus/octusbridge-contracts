...
describe('Test Staking Rewards', async function () {
    ...
    const userRewardRounds = async function (userData: Contract<FactorySource["UserData"]>) {
        return await userData.methods.getDetails({answerId: 0}).call().then(v => v.value0.rewardRounds.toString());
    }

    const userTokenBalance = async function (userData: Contract<FactorySource["UserData"]>) {
        return await userData.methods.getDetails({answerId: 0}).call().then(v => v.value0.token_balance.toString());
    }

    const pendingReward = async function (user_token_balance: string, user_reward_rounds: any) {
        return await stakingRoot.methods.pendingReward({user_token_balance: user_token_balance, user_reward_data: user_reward_rounds, answerId: 0}).call().then(v => v.value0);
    }
    ...
    const claimReward = async function(user: Account) {
        await locklift.transactions.waitFinalized(stakingRoot.methods.claimReward({
    ...
    const checkReward = async function(
        ...
        const user_data = await userData.methods.getDetails({answerId: 0}).call().then(v => v.value0);
        const user_rew_after = user_data.rewardRounds;
        const user_rew_balance_before = prevRewData[0].reward_balance;
        const user_rew_balance_after = parseInt(user_rew_after[0].reward_balance, 10);
    ...
    const withdrawTokens = async function(user: Account, withdraw_amount: number) {
        return await locklift.transactions.waitFinalized(stakingRoot.methods.withdraw({
    ...
    describe('Setup contracts', async function() {
        ...
        describe('Users', async function() {
            ...
            it('Deploy users token wallets + mint', async function() {
                ...
                const balance3 = await ownerWallet.methods.balance({answerId:0}).call();

                expect(balance1).to.be.equal(userInitialTokenBal, 'User ton token wallet empty');
                expect(balance2).to.be.equal(userInitialTokenBal, 'User ton token wallet empty');
                expect(balance3).to.be.equal(userInitialTokenBal, 'User ton token wallet empty');
            });
        });
    ...
    describe('Basic staking pipeline', async function () {
        ...
        describe('Multiple users farming 1 round', async function() {
            ...
            it('Users withdraw tokens', async function () {
                ...
                const reward1 = parseInt(user1_rew_after[0].reward_balance, 10) - parseInt(user1_rew_before[0].reward_balance, 10);

                const time_passed_1 = parseInt(user2_deposit_time, 10) - parseInt(user1_deposit_time, 10);
                const expected_reward_1 = rewardPerSec * time_passed_1;

                const time_passed_2 = parseInt(user1_withdraw_time, 10)  - parseInt(user2_deposit_time, 10);
                ...
                const reward2 = parseInt(user2_rew_after[0], 10) - parseInt(user2_rew_before[0], 10);

                const time_passed_21 = parseInt(user1_withdraw_time, 10)  - parseInt(user2_deposit_time, 10);
                const expected_reward_21 = rewardPerSec * 0.5 * time_passed_21;

                const time_passed_22 = parseInt(user2_withdraw_time, 10)  - parseInt(user1_withdraw_time, 10) ;
            ...
            it("Claim rewards (expect fail)", async function () {
                ...
                expect(user2_reward_before[0]).to.be.equal(user2_reward_after[0], "Claim reward fail");
            })
            ...
            it("Claim rewards", async function () {
                ...
                const user1_token_reward = Math.floor(Math.floor(parseInt(user1_reward, 10) * 1e10 / parseInt(round1_rewards_data.totalReward, 10)) * parseInt(round1_rewards_data.rewardTokens, 10) / 1e10);
                const user2_token_reward = Math.floor(Math.floor(parseInt(user2_reward, 10) * 1e10 / parseInt(round1_rewards_data.totalReward, 10)) * parseInt(round1_rewards_data.rewardTokens, 10) / 1e10);
                ...
                const user1_data_2 = await user1Data.methods.getDetails({answerId: 0}).call().then(v => v.value0);
        ...
        describe("Multiple users farming several rounds", async function() {
            ...
            it("User 2 deposit", async function () {
                ...
                const staking_details = await stakingRoot.methods.getDetails({answerId: 0}).call().then(v => v.value0);
                const last_reward_time = staking_details.lastRewardTime;
                const time_passed = parseInt(last_reward_time, 10) - parseInt(round2_start_time, 10) ;
            ...
            it("New reward round starts", async function () {
                ...
                const time_passed = parseInt(last_reward_time, 10) - parseInt(prev_reward_time, 10);
            ...
            it("Users withdraw tokens", async function () {
                ...
                const user1_details_1 = await user1Data.methods.getDetails({answerId: 0}).call().then(v => v.value0);
            ...
            it("Users claim tokens", async function() {
                ...
                for (const i of [...Array(reward_rounds.length).keys()]) {
                    const user1_round_reward = parseInt(user1_reward_data[i].reward_balance, 10);
                    const user2_round_reward = parseInt(user2_reward_data[i], 10);

                    const user1_token_reward = Math.floor(Math.floor(user1_round_reward * 1e10 / parseInt(reward_rounds[i].totalReward, 10) ) * parseInt(reward_rounds[i].rewardTokens, 10)  / 1e10);
                    const user2_token_reward = Math.floor(Math.floor(user2_round_reward * 1e10 / parseInt(reward_rounds[i].totalReward, 10) ) * parseInt(reward_rounds[i].rewardTokens, 10)  / 1e10);
