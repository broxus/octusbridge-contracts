...
let walletFabric: Contract<FactorySource["TestWalletFabric"]>;
...
const eth_addrs = [...Array(USERS_NUM).keys()].map(
  (i) => `0x${randomBytes(20).toString("hex")}`
);
...
describe.skip("Staking highload relay test", async function () {
  ...
  const deployAccounts = async function (user, keypairs) {
    ...
    for (const i of [...Array(Math.ceil(keypairs.length / slice_len)).keys()]) {
      ...
      await user.runTarget({
        ...
        params: {
          owners: _pubkeys,
          initial_balance: convertCrystal(
            RELAY_INITIAL_DEPOSIT + 10,
            "nano"
          ).toString(),
        },
      ...
      expect(_wallets.length).to.be.eq(
        _pubkeys.length,
        "Did not create wallets"
      );
  ...
  const waitForStakingEvent = async function (name, timeout = 60000) {
    ...
    while (sleep_time < timeout) {
      id_to_del = staking_events.findIndex(
        (elem, id, arr) => elem.name === name
      );
      if (id_to_del !== -1) {
        break;
      }
  ...
  describe("Setup contracts", async function () {
    ...
    describe("Token", async function () {
      ...
      it("Deploy users token wallets", async function () {
        ...
        expect(owner_balance.toString()).to.be.equal(
          userInitialTokenBal.toString()
        );

        await Promise.all(
          userTokenWallets.map(async (userWallet) => {
    ...
    describe("Staking", async function () {
      it("Deploy staking", async function () {
        ...
        const staking_wallet_addr = await stakingRoot.call({
          method: "tokenWallet",
        });
        ...
        const details = await stakingWallet.methods
          .getDetails({ answerId: 0 })
          .call()
          .then((v) => v.value0);
        expect(details.owner_address).to.be.equal(
          stakingRoot.address,
          "Wrong staking token wallet owner"
        );
        expect(details.receive_callback).to.be.equal(
          stakingRoot.address,
          "Wrong staking token wallet receive callback"
        );
        expect(details.root_address).to.be.equal(
      ...
      it("Sending reward tokens to staking", async function () {
        ...
        const staking_balance_stored = await stakingRoot.call({
          method: "rewardTokenBalance",
        });
      ...
      it("Setting relay config for testing", async function () {
        ...
        const relay_config = await stakingRoot.call({
          method: "getRelayConfig",
        });
  ...
  describe("Relay pipeline testing", async function () {
    describe("Standard case", async function () {
      it("Users deposit tokens", async function () {
        ...
        for (const i of [...Array(users.length).keys()]) {
          ...
          const user_data_bal = await user_data.call({
          ...
          expect(user_data_bal.toFixed(0)).to.be.equal(
            curUserDeposit.toFixed(0),
            `Bad deposit1 - user ${user.address}, idx - ${i}`
          );
          expect(user_bal.toFixed(0)).to.be.equal(
            expected_user_bal.toFixed(0),
            `Bad deposit2 - user ${user.address}, idx - ${i}`
          );
      ...
      it("Creating origin relay round", async function () {
        ...
        const reward_rounds = await stakingRoot.call({
          method: "rewardRounds",
        });
        ...
        expect(relays_count.toString()).to.be.equal(
          origin_users.length.toString(),
          "Bad relay round"
        );
        expect(reward_round_num.toString()).to.be.equal("0", "Bad relay round");

        const relay_rounds_data_0 = await stakingRoot.call({
          method: "getRelayRoundsDetails",
        });
        expect(relay_rounds_data_0.currentRelayRound.toString()).to.be.equal(
          "1",
          "Bad round installed in root"
        );

        const reward_rounds_new = await stakingRoot.call({
          method: "rewardRounds",
        });
        const expected_reward = round_reward.plus(
        ...
        await Promise.all(
          origin_users.map(async (user, idx) => {
            ...
            expect(relay.staker_addr).to.be.equal(
              user.address,
              "Relay creation fail - staker addr"
            );
            expect(relay.ton_pubkey.toString(16)).to.be.equal(
              user_pk.toString(16),
              "Relay creation fail - ton pubkey"
            );
            expect(relay.eth_addr.toString(16)).to.be.equal(
              user_eth.toString(16),
              "Relay creation fail - eth addr"
            );
            expect(relay.staked_tokens.toString()).to.be.equal(
              "1",
        ...
        const relay_rounds_data = await stakingRoot.call({
          method: "getRelayRoundsDetails",
        });
        expect(relay_rounds_data.originRelayRoundInitialized).to.be.equal(
          true,
          "Origin round not initialized"
        );
      });
      ...
      it("Users link relay accounts", async function () {
        for (const i of [...Array(USERS_NUM).keys()]) {
          ...
          const _user_eth_addr = await userData.call({
            method: "relay_eth_address",
          });

          const user_pk_expected = new BigNumber(user.keyPair.public, 16);
          const user_eth_addr_expected = new BigNumber(
            eth_addrs[i].toLowerCase(),
            16
          );

          expect(_user_pk.toString(16)).to.be.equal(
            user_pk_expected.toString(16),
            "Bad ton pubkey installed"
          );
          expect(_user_eth_addr.toString(16)).to.be.equal(
            user_eth_addr_expected.toString(16),
      ...
      it("Users confirm ton relay accounts", async function () {
        for (const i of [...Array(USERS_NUM).keys()]) {
          ...
          const confirmed_user = await userData.call({
            method: "ton_pubkey_confirmed",
          });
          expect(confirmed_user).to.be.equal(
            true,
            "Ton pubkey user not confirmed"
          );
        }
      });
      ...
      it("Users confirm eth relay accounts", async function () {
        for (const i of [...Array(USERS_NUM).keys()]) {
          ...
          const confirmed_user = await userData.call({
            method: "eth_address_confirmed",
          });
          expect(confirmed_user).to.be.equal(
            true,
            "Eth pubkey user not confirmed"
          );
        }
      });
      ...
      it("Users request relay membership", async function () {
        ...
        for (const i of [...Array(USERS_NUM).keys()].reverse()) {
          ...
          const user_token_balance = await userData.call({
            method: "token_balance",
          });

          const expected_ton_pubkey = `0x${user_pk
            .toString(16)
            .padStart(64, "0")}`;
          const expected_eth_addr = `0x${user_eth_addr
            .toString(16)
            .padStart(64, "0")}`;
          ...
          expect(_tokens.toString()).to.be.equal(
            user_token_balance.toString(),
            "Bad event - tokens"
          );
          expect(_ton_pubkey.toString()).to.be.equal(
            expected_ton_pubkey,
            "Bad event - ton pubkey"
          );
          expect(_eth_address.toString(16)).to.be.equal(
            expected_eth_addr,
            "Bad event - eth address"
          );
          expect(Number(_lock_until)).to.be.gte(
            Number(block_now),
            "Bad event - lock"
          );
        }
        ...
        const requests = await election.call({
          method: "getRequests",
          params: { limit: USERS_NUM },
        });
        for (const i of [...Array(USERS_NUM).keys()]) {
          ...
          expect(req.staker_addr).to.be.equal(
            user.address,
            "Bad request - staker addr"
          );
          expect(req.staked_tokens.toString()).to.be.equal(
            user_token_balance.toString(),
            "Bad request - token balance"
          );
          expect(req.ton_pubkey).to.be.equal(
            expected_ton_pubkey,
      ...
      it("Election ends, new round initialized", async function () {
        ...
        const stored_relays_count = await round.call({
          method: "relays_count",
        });
        const stored_reward_round_num = await round.call({
          method: "reward_round_num",
        });
        const stored_relays_installed = await round.call({
          method: "relays_installed",
        });
    ...
    describe("Not enough relay requests on election", async function () {
      it("New reward round starts", async function () {
        ...
        const last_reward_time = await stakingRoot.call({
          method: "lastRewardTime",
        });
      ...
      it("Users request relay membership", async function () {
        ...
        const user_pk = new BigNumber(user.keyPair.public, 16);
        const expected_ton_pubkey1 = `0x${user_pk
          .toString(16)
          .padStart(64, "0")}`;
        const user_eth = new BigNumber(user_eth_addr.toLowerCase(), 16);
        const block_now = tx.now + 30 * 24 * 60 * 60;

        const expected_eth_addr = `0x${user_eth
          .toString(16)
          .padStart(64, "0")}`;
        expect(_round_num1.toString()).to.be.equal(
          "3",
        ...
        expect(_eth_address1.toString(16)).to.be.equal(
          expected_eth_addr,
          "Bad event - eth address"
        );
      ...
      it("Election ends, not enough users participated, clone prev. round", async function () {
        ...
        const expected_reward = round_reward.plus(
          new BigNumber(reward_rounds[1].totalReward)
        );
        expect(expected_reward.toString()).to.be.equal(
