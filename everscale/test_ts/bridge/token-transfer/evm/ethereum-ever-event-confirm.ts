...
describe('Test ethereum everscale event confirm', async function() {
  ...
  describe('Initialize event', async () => {
    ...
    it('Setup event data', async () => {
      eventDataStructure.owner_addr = initializer.address.toString().replace('0:', '0x');
    ...
    it('Check encoded event data', async () => {
      ...
      expect(data.tokens)
        .to.be.equal(eventDataStructure.tokens, 'Wrong amount of tokens');

      expect(data.wid)
        .to.be.equal(eventDataStructure.wid, 'Wrong wid');
  ...
  describe('Confirm event', async () => {
    ...
    it('Check event proxy minted tokens', async () => {
      expect(await initializerTokenWallet.methods.balance({answerId: 0}).call())
        .to.be.equal(eventDataStructure.tokens, 'Wrong initializerTokenWallet balance');
    });
  });

  describe('Test Proxy', async () => {
    it('Test Proxy token transfer ownership', async () => {
      const token: Contract<FactorySource["TokenRoot"]> = await getTokenRoot(proxy.methods.getTokenRoot({answerId: 0}).call());

      expect(await token.methods.rootOwner({answerId: 0}).call()).to.be.equal(proxy.address, 'Wrong initial token owner');

      await proxy.methods.transferTokenOwnership({
      ...
      expect(await token.methods.rootOwner({answerId: 0}).call())
