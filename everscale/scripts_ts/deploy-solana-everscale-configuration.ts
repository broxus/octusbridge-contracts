...
const main = async () => {
  ...
  const SolanaEvent = await locklift.factory.getContractArtifacts(response.eventContract);
  
  const spinner = ora('Deploying Solana event configuration').start();

  const {contract: solanaEverscaleEventConfiguration}  = await locklift.factory.deployContract({
