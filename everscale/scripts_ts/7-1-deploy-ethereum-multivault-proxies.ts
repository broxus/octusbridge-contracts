...
const main = async () => {
  ...
  const { contract: proxyMultiVaultNative } =
  ...
  const { contract: proxyMultiVaultAlien } =
    await locklift.factory.deployContract({
  ...
  const setConfigurationTx = await proxyMultiVaultAlien.methods
    .setConfiguration({
      _config: {
  ...
  const transferOwnershipTx = await proxyMultiVaultAlien.methods
    .transferOwnership({
