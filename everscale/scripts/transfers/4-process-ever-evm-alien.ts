...
import { MultiVaultEverscaleEVMEventAlienAbi } from "../../build/factorySource";
...
const main = async () => {
  const tonEthEventConfiguration = locklift.factory.getDeployedContract(
    ...
    new Address(process.env.TON_ETH_ALIEN_EVENT_CONFIGURATION!),
  );
  ...
  const eventsToConfirm: Contract<MultiVaultEverscaleEVMEventAlienAbi>[] = [];
