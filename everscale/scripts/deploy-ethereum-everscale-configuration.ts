...
const main = async () => {
  ...
  const response = await prompts([
    ...
    },
    {
      ...
      choices: networks,
      initial:
        networks.map((n) => n.value).indexOf(options.chainId) >= 0
          ? networks.map((n) => n.value).indexOf(options.chainId)
          : 0,
    },
    {
      type: "text",
    ...
    await locklift.factory.deployContract({
      ...
      initParams: {
        ...
        networkConfiguration: {
          ...
          eventEmitter: new BigNumber(
            response.eventEmitter.toLowerCase()
          ).toFixed(),
