...
describe("Deposit Alien token with merging with custom token", async function () {
  ...
  it("Set merge pool in router", async () => {
    ...
    const pool = await mergeRouter.methods.getPool({ answerId: 0 }).call({ responsible: true });
  ...
  it("Check alien total supply is zero", async () => {
    ...
    expect(Number(totalSupply.value0)).to.be.equal(0, "Alien total supply should remain zero");
  });
  ...
  it("Check custom total supply", async () => {
    ...
    expect(Number(totalSupply.value0)).to.be.equal(
