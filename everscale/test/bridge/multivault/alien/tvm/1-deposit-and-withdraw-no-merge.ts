...
describe("Deposit and withdraw alien TVM token with no merging", () => {
  ...
  it("Burn alien TVM token in favor of proxy", async () => {
    ...
    // const events = await alienProxy.getPastEvents({filter: "TvmTvmAlien" as const});
    ...
    return expect(traceTree)
      ...
      .withNamedArgs({
        ...
        baseToken: "0:ae8cf02ded2f2b7fc083b3d33fdb7aef90c5f51dc918c082611dddd94dfed395",
        name: "Custom Giga Chad",
        symbol: 'CUSTOM_GIGA_CHAD',
        decimals: '9',
        nativeProxyWallet: "0:cb7929ed6b57177e477595f07707e15701f4c6aee51e3b714ff75a7726c835ed",
        sender: bridgeOwner.address,
        recipient: bridgeOwner.address,
        amount: (400 - 40).toString(),
        expectedGas: "0",
        remainingGasTo: bridgeOwner.address,
  ...
  it("Withdraw accumulated fee from bridgeTokenFee", async () => {
     ...
     expect(traceTree)
      .to.call("mint")
      .count(1)
      .withNamedArgs({amount: '85', recipient: bridgeOwner.address});
     ...
     return expect(+diff).to.be.equal(85);
  });
});
