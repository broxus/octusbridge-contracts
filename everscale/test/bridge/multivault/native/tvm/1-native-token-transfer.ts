...
let tvmTvmEventConfiguration: Contract<TvmTvmEventConfigurationAbi>;
let proxy: Contract<ProxyMultiVaultNative_V7Abi>;
...
describe.skip("Test TVM native multivault pipeline", async function () {
  ...
  it("Setup bridge", async () => {
    ...
    [, , , , proxy, tvmTvmEventConfiguration] = await setupNativeMultiVault(bridgeOwner, staking, trustlessVerifier);
  ...
  describe("Transfer native token from Everscale to EVM", async () => {
    ...
    it("Transfer tokens to the Native Proxy", async () => {
      ...
      eventContract = locklift.factory.getDeployedContract("MultiVaultEverscaleEVMEventNative", expectedEventContract);
    });
    ...
    it("Check event state before confirmation", async () => {
      ...
      expect(details._signatures).to.have.lengthOf(0, "Wrong amount of signatures");
    ...
    it("Check event data after mutation", async () => {
      ...
      expect(decodedData.chainId_.toString()).to.be.equal(chainId.toString(), "Wrong event decoded amount");
    ...
    it("Check mutated event data", async () => {
      ...
      expect(decodedData.token_wid).to.be.equal(
        tokenRoot.address.toString().split(":")[0],
        "Wrong event data token wid",
      );
      expect("0x" + new BigNumber(decodedData.token_addr).toString(16)).to.be.equal(
        `0x${tokenRoot.address.toString().split(":")[1]}`,
      ...
      expect(decodedData.chainId).to.be.equal(chainId.toString(), "Wrong event data amount");
    ...
    it("Check event confirmed", async () => {
      ...
      expect(details._signatures).to.have.lengthOf(requiredVotes, "Wrong amount of signatures");
  ...
  describe("Transfer native token from EVM to Everscale", async () => {
    ...
    it("Check event decoded data", async () => {
      ...
      expect(decodedData.token_.toString()).to.be.equal(tokenRoot.address.toString(), "Wrong event decoded data token");
      expect(decodedData.amount_).to.be.equal(amount.toString(), "Wrong event decoded data amount");
      expect(decodedData.recipient_.toString()).to.be.equal(
        initializer.address.toString(),
        "Wrong event decoded data recipient",
      );
      expect(decodedData.proxy_.toString()).to.be.equal(proxy.address.toString(), "Wrong event decoded data proxy");
    ...
    it("Check initializer token balance", async () => {
      ...
      expect(parseInt(balance.value0, 10)).to.be.equal(mintAmount, "Wrong initializer token balance");
    });
