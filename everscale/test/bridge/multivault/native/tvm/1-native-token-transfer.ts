...
describe("Test TVM native multivault pipeline", async function () {
    ...
    it('Check initializer token balance', async () => {
        ...
        initializerTokenWallet = locklift.factory.getDeployedContract(
            'TokenWallet',
    ...
    describe("Transfer native token from Everscale to EVM", async () => {
        ...
        it("Transfer tokens to the Native Proxy", async () => {
            ...
            const tx = await locklift.tracing.trace(initializerTokenWallet.methods
                .transfer({
                    amount,
        ...
        it("Check initializer token balance", async () => {
            ...
            expect(Number(balance.value0)).to.be.equal(
                mintAmount - amount,
        ...
        it("Check native proxy token balance", async () => {
            ...
            expect(Number(balance))
                .to.be.equal(amount, "Wrong initializer token balance");
        });
